...
namespace BLL.Services.UserServices
{
    internal class TokenGeneratorService : ITokenGeneratorService
    {
        ...
        public OptionalResult<Guid> GetIdFromBase64(string base64Id)
        {
            if (Guid.TryParse(Base64UrlEncoder.Decode(base64Id), out Guid result))
            {
                return new OptionalResult<Guid>(result);
            }

            return new OptionalResult<Guid>(false, "Invalid uidb64");
        }
